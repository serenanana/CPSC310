 ...
 */
/// <reference path="../controller/QueryController.ts" />
        ...
        return new Promise(function (fulfill, reject) {
            try {
                ...
                that.rest.get(/.*/, restify.serveStatic({
                    directory: __dirname,
                    default: '/views/courses.html',
                }));

                // that.rest.get(/\/docs\/current\/?.*/, restify.serveStatic({
                //     directory: './documentation/v1',
                //     default: 'index.html'
                // }));

                that.rest.get(/\/rooms.html\//, restify.serveStatic({
                    directory: __dirname,
                    default: '/views/rooms.html'
                }));

                that.rest.get(/\/scheduler.html\//, restify.serveStatic({
                    directory: __dirname,
                    default: '/views/scheduler.html'
                }));

                that.rest.get(/\/space.html\//, restify.serveStatic({
                    directory: __dirname,
                    default: '/views/space.html'
                }));
                ...
                that.rest.put('/dataset/:id', function (req: restify.Request, res: restify.Response, next: restify.Next) {
                    ...
                    IF.addDataset(req.params.id, dataStr)
                        ...
                        .catch(function (insightError: InsightResponse){
                            res.json(insightError.code,insightError.body);
                        });
                    return next;
                });
                ...
                // that.rest.post('/query_with_distance', function (req: restify.Request, res: restify.Response, next: restify.Next) {
                //     //console.log("@@@@@");
                ...
                //
                //     IF.performQuery(req.body.query)
                //         .then(function (insightResponse: InsightResponse) {
                //             // first filter by the distance
                //
                //
                //             res.json(insightResponse.code, insightResponse.body);
                //         })
