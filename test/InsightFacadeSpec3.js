...
describe('InsightFacadeSpec3', function () {
    ...
    it('Query A', function (done) {
        IF.performQuery({
            ...
            chai_1.expect(JSON.stringify(data.body)).to.equal(JSON.stringify({
                "render": "TABLE",
                "result": [
                    {
    ...
    it('Stringer: Should be able to query with many APPLY keys and sort over them.', function (done) {
        IF.performQuery({
            ...
            chai_1.expect(data.code).to.equal(200);
            chai_1.expect(JSON.stringify(data.body)).to.equal(JSON.stringify({ "render": "TABLE", "result": [{ "rooms_shortname": "OSBO", "maxSeats": 442, "minSeats": 442, "avgSeats": 442, "sumSeats": 442, "countSeats": 1 }, { "rooms_shortname": "HEBB", "maxSeats": 375, "minSeats": 375, "avgSeats": 375, "sumSeats": 375, "countSeats": 1 }, { "rooms_shortname": "LSC", "maxSeats": 350, "minSeats": 350, "avgSeats": 350, "sumSeats": 700, "countSeats": 1 }] }));
            done();
        }).catch(function (err) {
            console.log("QueryA");
            console.log(err);
    ...
    it('COUNT supports both string and numbers', function (done) {
        IF.performQuery({
            ...
            chai_1.expect(data.code).to.equal(200);
            console.log(JSON.stringify(data.body));
            chai_1.expect(JSON.stringify(data.body)).to.equal(JSON.stringify({ "render": "TABLE", "result": [{ "rooms_shortname": "WOOD", "countSeats": 1 }, { "rooms_shortname": "HEBB", "countSeats": 1 }, { "rooms_shortname": "WESB", "countSeats": 1 }, { "rooms_shortname": "OSBO", "countSeats": 1 }, { "rooms_shortname": "LSC", "countSeats": 1 }, { "rooms_shortname": "ESB", "countSeats": 1 }, { "rooms_shortname": "CIRS", "countSeats": 1 }] }));
            done();
        }).catch(function (err) {
            chai_1.expect.fail();
            done();
...
//# sourceMappingURL=InsightFacadeSpec3.js.map